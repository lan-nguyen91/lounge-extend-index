...
exports.saveOptionsKeys = exports.schemaConfigOptionKeys = exports.defaultOptions = exports.SetterError = undefined;
...
exports.isKeyNotFound = isKeyNotFound;
...
var SetterError = exports.SetterError = function SetterError(errorMessage, setValue, originalValue, fieldSchema) {
...
function typecast(value, originalValue, properties) {
  ...
  if (properties.ref === true && properties.objectType) {
    if (_lodash2.default.isString(value) || _lodash2.default.isObject(value)) {
      return value;
    }
  ...
  switch (properties.type) {
    case 'string':
      ...
      if (_lodash2.default.isObject(value) || _lodash2.default.isArray(value)) {
      ...
      if (_lodash2.default.isArray(properties.enum) && properties.enum.indexOf(value) === -1) {
    ...
    case 'number':
      ...
      if (_lodash2.default.isArray(value) || _lodash2.default.isObject(value) || !isNumeric(value)) {
    ...
    case 'array':
      ...
      if (_lodash2.default.isObject(value)) {
        value = _lodash2.default.toArray(value);
      }
      ...
      if (_lodash2.default.isFunction(originalValue.set)) {
        originalValue.set(value);
      } else {
        originalValue.length = 0;
        _lodash2.default.each(value, function (arrayValue) {
    ...
    case 'date':
      ...
      if (!_lodash2.default.isDate(value) && !_lodash2.default.isString(value) && !_lodash2.default.isNumber(value)) {
      ...
      if (value === 'Invalid Date' || !_lodash2.default.isDate(value)) {
...
var schemaConfigOptionKeys = exports.schemaConfigOptionKeys = ['keyPrefix', 'keySuffix', 'refIndexKeyPrefix', 'delimiter', 'minimize'];
var saveOptionsKeys = exports.saveOptionsKeys = ['storeFullReferenceId', 'storeFullKey', 'refIndexKeyPrefix', 'waitForIndex', 'toObject.minimize'];
...
function expandValues(valueList) {
  var retVal = [];
  _lodash2.default.each(valueList, function (val) {
    if (Array.isArray(val)) {
      var expVal = expandValues(val);
      var intRet = [];
      _lodash2.default.each(expVal, function (v) {
...
function processValue(initialValue) {
  var finalVal = null;
  ...
  if (initialValue && (typeof initialValue === 'undefined' ? 'undefined' : _typeof(initialValue)) === 'object' && initialValue._isBaseModel) {
    finalVal = initialValue.getDocumentKeyValue(initialValue.config.storeFullReferenceId);
  } else if (Array.isArray(initialValue)) {
    finalVal = _lodash2.default.map(initialValue, function (v) {
      if (v) {
        if (typeof v === 'string') {
          return v;
        }
        ...
        if ((typeof v === 'undefined' ? 'undefined' : _typeof(v)) === 'object' && v._isBaseModel) {
          return v.getDocumentKeyValue(v.config.storeFullReferenceId);
        }
      }
    });

    finalVal = _lodash2.default.compact(finalVal);
    finalVal = finalVal.sort();
  } else if (_lodash2.default.isUndefined(initialValue) || _lodash2.default.isNull(initialValue)) {
    finalVal = null;
  }
  return finalVal;
}