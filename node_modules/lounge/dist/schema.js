...
var schemaUtils = _interopRequireWildcard(_schema);
...
var trailingDigitReg = new RegExp('^\\d+$');
...
var Schema = function () {
  ...
  function Schema(descriptor) {
    ...
    if (!this.key.prefix && _lodash2.default.isString(this.options.keyPrefix)) {
      this.key.prefix = this.options.keyPrefix;
    }

    if (!this.key.suffix && _lodash2.default.isString(this.options.keySuffix)) {
  ...
  _createClass(Schema, [{
    ...
    value: function _applyDocumentKey(descriptor) {
      ...
      for (var prop in descriptor) {
        if (descriptor.hasOwnProperty(prop)) {
          if (utils.isPlainObject(descriptor[prop]) && descriptor[prop].key === true) {
            ...
            if (descriptor[prop].type && descriptor[prop].type !== String && descriptor[prop].type !== Number && descriptor[prop].type !== 'number' && descriptor[prop].type !== 'string') {
            ...
            if (_lodash2.default.isString(descriptor[prop].prefix)) {
              this.key.prefix = descriptor[prop].prefix;
            }

            if (_lodash2.default.isString(descriptor[prop].suffix)) {
              this.key.suffix = descriptor[prop].suffix;
            }

            if (_lodash2.default.isBoolean(descriptor[prop].generate)) {
              this.key.generate = descriptor[prop].generate;
            }

            this.descriptor[this.key.docKeyKey] = _normalize.normalizeProperties.call(this, descriptor[prop], this.key.docKeyKey);
          }
        }
      }
      ...
      if (!docKeyFound && !this.key.docKeyKey) {
        ...
        this.descriptor[this.key.docKeyKey] = _normalize.normalizeProperties.call(this, String, this.key.docKeyKey);
      }
    }
    ...
    key: '_getIndexes',
    value: function _getIndexes(descriptor) {
      var _this2 = this;
    ...
    key: 'index',
    value: function index(prop) {
      var options = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : {};

      var indexType = options.indexType || 'single';
      var indexName = options.indexName;
      var compound = false;
      if (Array.isArray(prop)) {
        if (!indexName) {
          var indexProps = _lodash2.default.map(prop, function (p) {
            if (trailingDigitReg.test(p)) {
              p = p[p.length - 2];
            }
            p = p.replace('.', '_');
            return _inflection2.default.singularize(p);
          });
        ...
        if (!indexName) {
          ...
          indexName = _inflection2.default.singularize(indexName);
        }
      }

      var indexData = { path: prop, name: indexName, indexType: indexType, compound: compound };
    ...
    key: 'method',
    value: function method(name, func) {
      if (_lodash2.default.isPlainObject(name)) {
        for (func in name) {
        ...
        } else if (!_lodash2.default.isFunction(func)) {
    ...
    key: 'static',
    value: function _static(name, func) {
      if (_lodash2.default.isPlainObject(name)) {
        for (func in name) {
          if ({}.hasOwnProperty.call(name, func)) {
    ...
    key: '_hook',
    value: function _hook(hook, name, fn) {
    ...
    key: 'add',
    value: function add(key, descriptor) {
      var _this3 = this;
      ...
      if (key && descriptor) {
        ...
        this.descriptor[key] = _normalize.normalizeProperties.call(this, descriptor, key);
      } else if ((typeof key === 'undefined' ? 'undefined' : _typeof(key)) === 'object' && !descriptor) {
        this._getRefs(key);
        this._getIndexes(key);

        _lodash2.default.each(key, function (properties, index) {
          _this3.descriptor[index] = _normalize.normalizeProperties.call(_this3, properties, index);
        });
      }
    ...
    key: '_cloneProp',
    value: function _cloneProp(other, prop, add) {
      if (other && other[prop]) {
        var p = void 0;
        for (p in other[prop]) {
          if (other[prop].hasOwnProperty(p) && !this[prop][p]) {
            if (add) {
              this.add(p, (0, _clone2.default)(other.descriptor[p]));
            } else {
      ...
      if (other && other instanceof Schema) {
        ...
        for (k in other.options) {
          if (other.options.hasOwnProperty(k)) {
            var ours = this.get(k);
            var theirs = other.options[k];
            if (_lodash2.default.isUndefined(ours)) {
              this.set(k, (0, _clone2.default)(theirs));
            } else if (_lodash2.default.isPlainObject(ours) && _lodash2.default.isPlainObject(theirs)) {
              var dest = (0, _clone2.default)(theirs);
              this.set(k, _lodash2.default.merge(dest, ours));
            }
          }
        }

        _lodash2.default.forEach(_lodash2.default.values(other.hooks), function (hook) {
          _lodash2.default.forEach(hook.fns, function (fn) {
            _this4[hook.hook](hook.name, fn);
          });
        });
      }
    ...
    value: function getDocumentKeyValue(id, full) {
      if (_lodash2.default.isString(id)) {
        var prefix = _lodash2.default.isString(this.key.prefix) ? this.key.prefix : '';
        var suffix = _lodash2.default.isString(this.key.suffix) ? this.key.suffix : '';
    ...
    key: 'getRefKey',
    value: function getRefKey(name, v) {
    ...
    key: 'hasRefPath',
    value: function hasRefPath(path) {
      var ret = false;
      if (this.refs && path) {
        path = path.toLowerCase();
        for (var key in this.refs) {
          var resolvedPath = this.refs[key].path.toLowerCase();
